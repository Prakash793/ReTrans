...
const App: React.FC = () => {
  ...
  const handleKeySelection = async () => {
    try {
      ...
      // Per system instructions, assume selection success immediately to allow SDK to fetch updated key from env
      setHasKey(true);
  ...
  const handleFileUpload = async (file: File) => {
    setState(prev => ({ 
      ...
      isProcessing: true, 
      progress: 10, 
      statusMessage: 'Analyzing Structural Integrity...',
      error: null, 
    ...
    try {
      ...
      setState(prev => ({ 
        ...
        progress: 30,
        statusMessage: processed.chunks.length > 0 ? 'Structure Mapped.' : 'Multimodal Vision Ready.',
        isProcessing: false,
  ...
  const startTranslation = async () => {
    // If key is missing or invalid string, force the browser selection dialog
    if (!process.env.API_KEY || process.env.API_KEY === 'undefined' || process.env.API_KEY.length < 5) {
      await handleKeySelection();
      // Proceed immediately as per SDK instructions for environment injection
    }
    ...
    setState(prev => ({ 
      ...
      error: null, 
      progress: 40, 
      statusMessage: isImageBased ? 'Vision Fidelity Scanning...' : 'Contextual Neural Analysis...' 
    }));
    ...
    try {
      if (isImageBased && state.originalFileData) {
        ...
        setState(prev => ({
          ...
          isProcessing: false,
          progress: 100,
          statusMessage: 'Neural Translation Completed.'
        }));
      }
    } catch (err: any) {
      ...
      setState(prev => ({ 
        ...
        isProcessing: false, 
        error: isAuthError 
          ? 'An active API Key from a paid Google Cloud project is required for Enterprise Fidelity models.' 
          : (errorMsg || 'Processing Interrupted.'), 
        progress: 0, 
        statusMessage: 'Link Failure.' 
      }));
    }
  };
  ...
  const handleDownloadDoc = (mode: 'original' | 'translated') => {
    ...
    const htmlHeader = `
      ...
      <head>
        ...
        <style>
          ...
          td, th { border: 1px solid #cbd5e0; padding: 10pt; font-size: 10pt; vertical-align: top; }
          .bold { font-weight: bold; }
          .italic { font-style: italic; }
          .underline { text-decoration: underline; }
          .checkbox { font-family: 'Segoe UI Symbol', sans-serif; margin-right: 8pt; font-size: 14pt; }
          .empty-line { height: 16pt; width: 100%; }
        </style>
      </head>
      <body>
    `;
    ...
    const suffix = mode === 'original' ? '_MIRROR' : '_TRANSLATED';
            ...
            <div className="text-center space-y-10 max-w-5xl mx-auto">
              <div className="flex justify-center mb-6">
                <span className="px-6 py-2 rounded-2xl text-[11px] font-black uppercase tracking-[0.4em] bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-100 dark:border-blue-800 shadow-sm">
                  Neural Fidelity Engine
                </span>
              </div>
              <h1 className="text-6xl md:text-8xl font-black tracking-tighter leading-none font-serif text-[#001a33] dark:text-white">
                Contextual.<br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-700 via-blue-500 to-indigo-600">Pure Mirror.</span>
              </h1>
              <p className="text-xl text-slate-500 dark:text-slate-400 max-w-3xl mx-auto font-medium leading-relaxed">
                Experience translation that preserves the soul and structure of your enterprise documents.
              </p>
            </div>
              ...
              {!hasKey && (
                <div className="mb-8 p-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-4 text-amber-800 dark:text-amber-400">
                    ...
                    <div>
                      <p className="font-bold text-sm">Engine Standby</p>
                      <p className="text-xs opacity-80">ReTrans requires an enterprise API key to maintain document fidelity.</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noopener" className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-amber-700 hover:underline">
                      Billing Guide <ExternalLink className="w-3 h-3" />
                    </a>
                ...
                {inputMode === 'file' ? <FileUploader onUpload={handleFileUpload} /> : (
                  <div className="h-full flex flex-col gap-6">
                    ...
                    <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
                      <button onClick={() => setState(p => ({...p, originalFileName: 'Input', chunks: rawText.split('\n').map((l,i)=>({id:`t-${i}`,type:l.trim()?'paragraph':'empty-line',originalText:l}))}))} className="bg-blue-600 text-white px-10 py-4 rounded-xl font-black uppercase tracking-widest shadow-xl flex items-center gap-3">Initialize <ArrowRight className="w-5 h-5" /></button>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col xl:flex-row items-center justify-between gap-8">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="bg-white dark:bg-slate-800 p-3 px-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm min-w-[160px] relative">
                    <label className="text-[9px] font-black uppercase tracking-[0.3em] text-indigo-600 block mb-0.5">Translation Tone</label>
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <button onClick={toggleSource} className={`flex items-center gap-2 px-5 py-3 rounded-xl font-bold transition-all border ${showSource ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                    {showSource ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />} {showSource ? 'Fidelity Focus' : 'Show Source'}
                  </button>
                  ...
                  <button onClick={startTranslation} disabled={state.isProcessing} className="bg-blue-600 text-white px-10 py-3 rounded-xl font-black uppercase tracking-widest shadow-lg flex items-center gap-3 hover:bg-blue-700 transition-all">
                    ...
                    {state.isProcessing ? 'Thinking...' : 'Start Translation'}
                  </button>
            ...
            <div className={`grid gap-8 min-h-[75vh] transition-all duration-500 ${showSource ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1 max-w-4xl mx-auto'}`}>
              {showSource && (
                <div className="flex flex-col bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-lg overflow-hidden animate-in slide-in-from-left-4 duration-500">
                  <div className="px-8 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50">
                    <h3 className="font-black text-[10px] uppercase tracking-[0.3em] text-slate-400">Original Document</h3>
                    {state.chunks.length > 0 && (
                      <button 
                      ...
                      >
                        <ScanText className="w-3.5 h-3.5" /> EXPORT ORIGINAL
                      </button>
                    )}
                  </div>
              ...
              <div className="flex flex-col bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden relative transition-all duration-500">
                <div className="px-8 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-blue-50/10">
                  <h3 className="font-black text-[10px] uppercase tracking-[0.3em] text-blue-600">Translated Result</h3>
                  {state.progress === 100 && (
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-10">
                  {state.error ? (
                    <div className="h-full flex flex-col items-center justify-center text-center p-10 space-y-4">
                      ...
                      <h4 className="text-xl font-bold">Engine Interruption</h4>
                      ...
                      <div className="flex flex-col gap-4 items-center mt-6">
                        <div className="flex gap-4">
                          ...
                          <button onClick={handleKeySelection} className="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold text-sm flex items-center gap-2 shadow-lg hover:bg-blue-700">
                            <Key className="w-4 h-4" /> Link Key
                          </button>
                        </div>
                      </div>
                    </div>
